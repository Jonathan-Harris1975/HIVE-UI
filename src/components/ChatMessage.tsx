    ...
    <article className={`group flex gap-3 sm:gap-4 ${assistant ? '' : 'justify-end'}`} aria-busy={Boolean(message.pending)}>
      ...
      <div className={`min-w-0 max-w-[88%] ${assistant ? 'flex-1' : ''}`}>
            ...
            <>
              {message.content ? <MarkdownMessage content={message.content} /> : message.pending ? (
                <div className="flex items-center gap-2 text-slate-500" role="status" aria-live="polite">
                  <LoaderCircle className="h-4 w-4 animate-spin" /> HIVE is thinking
                </div>
              ) : null}
        ...
        <div className={`mt-1.5 flex items-center gap-1 transition sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100 ${assistant ? 'justify-start' : 'justify-end'}`}>
          <button type="button" onClick={() => void copyMessage()} disabled={!message.content} className="inline-flex h-8 items-center gap-1.5 rounded-lg px-2 text-[11px] text-slate-600 hover:bg-white/5 hover:text-slate-300 disabled:opacity-30" aria-label="Copy message">
            {copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />} {copied ? 'Copied' : 'Copy'}
          </button>
          <button type="button" onClick={() => onInspect(message)} className="inline-flex h-8 items-center gap-1.5 rounded-lg px-2 text-[11px] text-slate-600 hover:bg-white/5 hover:text-cyan-200" aria-label="Inspect message details">
