...
test('repository overview merges estate freshness into repository cards and keeps bounded refresh-all polling', () => {
  ...
  assert.match(apiTypes, /export interface RepositoryBulkUploadResponse/)
  assert.match(apiTypes, /export interface RepositoryRefreshResult/)
})

test('repository intelligence separates 12 percent work scope from quality and supports multi-pass execution', () => {
  const intelligence = source('src/pages/RepositoryIntelligencePage.tsx')
  const apiTypes = source('src/types/api.ts')

  assert.match(intelligence, /Single pass/)
  assert.match(intelligence, /Multi-pass/)
  assert.match(intelligence, /each pass remains capped at 12%/i)
  assert.match(intelligence, /The 12% value is a per-work-pass file-change budget/)
  assert.match(intelligence, /execution_mode: executionMode/)
  assert.match(intelligence, /max_work_passes: maxWorkPasses/)
  assert.match(intelligence, /Work-pass progress/)
  assert.match(intelligence, /Model attempts/)
  assert.match(intelligence, /Council reviews/)
  assert.match(intelligence, /Per-pass ledger/)
  assert.match(intelligence, /Remaining native-CI verification/)
  assert.match(intelligence, /static Repository QA is not equivalent to this repository's native CI/)
  assert.match(apiTypes, /export interface RepositoryImprovementPassLedgerEntry/)
  assert.match(apiTypes, /export interface RepositoryImprovementScope/)
  assert.match(apiTypes, /configured_change_ratio\?: number/)
})
